...
echo "$($PSQL "SELECT ROUND(SUM(winner_goals) * 1.00 / COUNT(games), 2) FROM games")"
...
echo "$($PSQL "SELECT DISTINCT(name) FROM teams RIGHT JOIN games ON teams.team_id = games.winner_id OR teams.team_id = games.opponent_id WHERE round='Quarter-Final' ORDER BY name")"
